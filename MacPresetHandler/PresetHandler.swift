...
class PresetHandler: ObservableObject {
    ...
    private func loadPresetsFromFile() -> [Preset]? {
        ...
        do {
            ...
            do {
                ...
                print("Successfully decoded as new format")
                return presets
            } catch {
                print("Failed to decode as new format: \(error)")
    ...
    private func getDocumentsDirectory() -> URL? {
        // First try to get the documents directory
        if let documentsPath = fileManager.urls(for: .documentDirectory, in: .userDomainMask).first {
            print("Using documents directory: \(documentsPath)")
            return documentsPath
        }
        
        // Fallback to the app bundle directory for development
        if let bundlePath = Bundle.main.resourceURL {
            print("Using bundle directory: \(bundlePath)")
            return bundlePath
        }
        
        print("Could not find any suitable directory")
        return nil
    }
